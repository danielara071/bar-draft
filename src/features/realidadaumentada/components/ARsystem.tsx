  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex flex-col items-center gap-2">
        ...
        {/* Indicador de búsqueda — solo si hay objetos cerca y ninguno seleccionado */}
        {nearbyCount > 0 && !selected && (
          <div className="flex items-center gap-2 rounded-full border border-white/8 bg-black/50 backdrop-blur-sm px-4 py-1.5">
            {isMobile ? (
              <>
                <MobileRotateIcon />
                <span className="font-sans text-[11px] text-white/60">
                  Gira a tu alrededor para encontrar el objeto
                </span>
              </>
            ) : (
              <>
                <span className="font-sans text-[13px] text-white/50">◄</span>
                <span className="font-sans text-[11px] text-white/60">
                  Usa las flechas para buscar el objeto
                </span>
                <span className="font-sans text-[13px] text-white/50">►</span>
              </>
            )}
...
function MobileRotateIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
      <path
        d="M12 2a10 10 0 0110 10M12 2v4M12 2l3 3M12 2L9 5"
      ...
      />
      <path
        d="M12 22a10 10 0 01-10-10M12 22v-4M12 22l-3-3M12 22l3-3"
        stroke="rgba(255,255,255,0.5)"
        strokeWidth="1.8"
        strokeLinecap="round"
        strokeLinejoin="round"
