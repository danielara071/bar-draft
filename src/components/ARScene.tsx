...
const MY_LAT = 25.6507150
const MY_LNG = -100.2897953
const WORLD_OBJECTS: WorldObject[] = [
  { id: 'w1', label: 'Tesoro',   color: '#FFD700', lat: MY_LAT + 0.00010, lng: MY_LNG           },
  { id: 'w2', label: 'Cristal',  color: '#FF4444', lat: MY_LAT,           lng: MY_LNG + 0.00010 },
  { id: 'w3', label: 'Gema',     color: '#44FFCC', lat: MY_LAT,           lng: MY_LNG - 0.00010 },
  { id: 'w4', label: 'Portal',   color: '#4488FF', lat: MY_LAT - 0.00010, lng: MY_LNG           },
]
...
function getBearing(userLat: number, userLng: number, objLat: number, objLng: number): number {
...
export default function ARScene() {
  ...
  const filterNearbyObjects = useCallback((coords: UserCoords) => {
    const nearby = WORLD_OBJECTS.filter((obj) =>
...
useEffect(() => {
  ...
  useEffect(() => {
    if (!userCoords || !containerRef.current || nearbyObjects.length === 0) return
    ...
    nearbyObjects.forEach((obj) => {
      ...
      box.addEventListener('click', () => {
        setSelected({ id: obj.id, label: obj.label, distance: Math.round(distance) })
        box.setAttribute('animation__click',
...
const styles: Record<string, React.CSSProperties> = {
  ...
  panel: {
    ...
    textAlign: 'center', zIndex: 999, minWidth: 240,
  },
  panelTitle: { fontSize: 22, fontWeight: 700, margin: 0 },
  panelSub:   { fontSize: 13, opacity: 0.5, margin: '4px 0 16px' },
  closeBtn: {
    background: '#fff', color: '#000', border: 'none',
    borderRadius: 10, padding: '10px 28px', cursor: 'pointer', fontWeight: 700,
  },
  spinner: {
    width: 40, height: 40,
